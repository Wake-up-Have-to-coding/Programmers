...
class Solution {
    public int[] solution(String[] genres, int[] plays) {
        ...
        HashMap<Integer, Integer> real_plyas_index = new HashMap<Integer,Integer>();
        ...
        for(int j = 0; j < sort_genres.length; j++){
            ...
            //장르에 맞는 플레이 수에 대한 index와 play_num을 real_plyas_index에 넣는다.
            for(int k = 0 ; k < real_genres_index.size() ; k++){
                real_plyas_index.put(real_genres_index.get(k),plays[real_genres_index.get(k)]);
                //System.out.println(real_plyas_index);
            }
            int count = (int) genres_index.entrySet().stream().filter(entry -> entry.getValue().equals(value)).count();
            if(count > 2) count = 2;
            for (int l = 0; l < count ; l++){
                int max = Collections.max(real_plyas_index.values());
                before_answer_list = real_plyas_index.entrySet().stream().filter(e -> e.getValue() == max).map(Map.Entry::getKey).collect(Collectors.toList());
                answer_list.add(before_answer_list.get(0));
                real_plyas_index.remove(before_answer_list.get(0));
                //System.out.println(real_plyas_index.entrySet().stream().filter(e -> e.getValue() == max).map(Map.Entry::getKey).collect(Collectors.toList()));
                //System.out.println(l);
            }
            System.out.println(answer_list);
            real_plyas_index.clear();
        }
        ...
        //System.out.println(real_plyas_index.entrySet().stream().filter(Collections.max(real_plyas_index.values()).getKey()));
        return answer;
    }
}
```
